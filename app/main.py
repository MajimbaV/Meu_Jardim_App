...
def main(page: ft.Page):
    # --- DEFINIÇÃO DO CAMINHO DE CONFIGURAÇÃO (compatível com Android) ---
    config_dir = None
    try:
    ...
    # --- CRIAÇÃO DOS BLOCOS DE AUTOMAÇÃO (COM AJUSTES DE LAYOUT) ---
    def criar_bloco_automacao(pino_rele, switch_componente):
        ...
        for d in dias_semana:
            ...
            container_chip = ft.Container(
                ...
                alignment="center",
                padding=ft.padding.symmetric(horizontal=4, vertical=6),  # reduzido
                width=38,   # fixo
                height=28,
                border_radius=6,
        ...
        # Layout interno mais compacto e com scroll
        return ft.ExpansionTile(
            title=ft.Text("Configurar Temporizador e Agenda", size=13, color="blue200"),
            maintain_state=True,
            controls=[
                ft.Container(
                    padding=ft.padding.all(8),  # padding reduzido
                    bgcolor="black26",
                    border_radius=8,
                    content=ft.Column(
                        [
                            ...
                            ft.Row(
                                lista_botoes, 
                                wrap=False,  # não quebra, ajusta-se à largura
                                alignment="center", 
                                spacing=4
                            )
                        ],
                        spacing=6,  # espaçamento reduzido
                        tight=True,  # evita expansão excessiva
                    )
                )
            ]
        )
